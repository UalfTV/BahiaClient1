window.BCOriginalRenderer = function(API, params){
  ...
  this.defineVariable({ name: "showTeamColors", description: "Show team colors?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "showAvatars", description: "Show player avatars?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "showPlayerIds", description: "Show player ids?", type: VariableType.Boolean, value: false });
  this.defineVariable({ name: "resolutionScale", description: "Resolution Scale", type: VariableType.Number, value: 1, range: { min: 0, max: Infinity, step: 0.01 } });
  this.defineVariable({ name: "followPlayerId", description: "Id of the player that the camera will follow", type: VariableType.Integer, value: 0 });
  this.defineVariable({ name: "restrictCameraOrigin", description: "Restrict camera origin to view bounds?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "followMode", description: "Follow camera enabled?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "showChatIndicators", description: "Show Chat Indicators?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "showFPS", description: "Show FPS counter?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "showInputLag", description: "Show Input Lag counter?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "showNetGraph", description: "Show network stats (ping/jitter/packet loss) graph?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "drawBackground", description: "Draw Background?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "squarePlayers", description: "Draw Players as squares?", type: VariableType.Boolean, value: false });
  this.defineVariable({ name: "currentPlayerDistinction", description: "Hide current player's name and draw halo around current player?", type: VariableType.Boolean, value: true });
  this.defineVariable({ name: "showInvisibleSegments", description: "Show invisible segments?", type: VariableType.Boolean, value: false });
  this.defineVariable({ name: "showVertices", description: "Show vertices?", type: VariableType.Boolean, value: false });
  this.defineVariable({ name: "generalLineWidth", description: "The line width of everything except discs and texts on screen.", type: VariableType.Number, value: 3, range: { min: 0, max: 100, step: 0.01 } });
  this.defineVariable({ name: "discLineWidth", description: "The line width of discs.", type: VariableType.Number, value: 4, range: { min: 0, max: 100, step: 0.01 } });
  this.defineVariable({ name: "displayMode", description: "Display mode (windowed, borderless, exclusive)", type: VariableType.String, value: 'windowed' });
  this.defineVariable({ name: "resolution", description: "Game target resolution", type: VariableType.String, value: 'native' });
  this.defineVariable({ name: "playerAvatarTexturePath", description: "texture path", value: null });
  ...
  const LanguageData = {
    "GB": ["Time is", "Up!", "Red is", "Victorious!", "Red", "Scores!", "Blue is", "Victorious!", "Blue", "Scores!", "Game", "Paused"],
    "TR": ["Süre", "Doldu!", "Kırmızı Takım", "Kazandı!", "Kırmızı Takım", "Gol Attı!", "Mavi Takım", "Kazandı!", "Mavi Takım", "Gol Attı!", "Oyun", "Durduruldu"],
    "PT": ["O tempo", "Acabou!", "O vermelho é", "Vitorioso!", "O vermelho", "Marca!", "O azul é", "Vitorioso!", "Pontuações", "Azuis!", "Jogo em", "Pausa"],
  ...
  function loadImages(playerAvatar){
    ...
    if (_lastAvatarUrl === playerAvatar && avatarImage) return;
  ...
  function redrawJoint({ gr, dx, dy, color }){
    gr.moveTo(0, 0);
    gr.lineTo(dx, dy);
    gr.stroke({ color: color, width: thisRenderer.generalLineWidth, alignment: 0.5 });
  };
  ...
  function redrawPlayerDisc(discInfo, teamColors, disc, player) {
    ...
    if (thisRenderer.playerAvatarTexturePath && player.id == thisRenderer.followPlayerId){
      ...
      baked = bakeTextureFill(half, half);
    }

    if (baked){
      if (discInfo.texture && !discInfo.texture.destroyed) {
        try { discInfo.texture.destroy(true); } catch (e) {}
      }
      discInfo.texture = baked.texture;
      gr.rotation = 0;
      gr.rect(x, -half, size, size);
      gr.fill({ texture: baked.texture, matrix: baked.matrix });
      discInfo.texturePath = thisRenderer.playerAvatarTexturePath;
    } else {
  ...
  function redrawHalo(){
    ...
    grHalo.stroke({ color: 0xffffff, width: thisRenderer.generalLineWidth, alignment: 0.5, alpha: 0.3 });
  }
  ...
  function _createDiscGraphics(discObj) {
    ...
    if (discObj.playerId !== null && discObj.playerId !== undefined) {
      ...
      if (thisRenderer.squarePlayers)
        ...
        playerStroke.circle(0, 0, discObj.radius);
      playerStroke.stroke({ width: thisRenderer.discLineWidth - 2, color: 0x000000 });

      kickGlow = new PIXI.Graphics();
      if (thisRenderer.squarePlayers)
        kickGlow.rect(-discObj.radius, -discObj.radius, 2 * discObj.radius, 2 * discObj.radius);
      else
        kickGlow.circle(0, 0, discObj.radius);
      kickGlow.stroke({ width: thisRenderer.discLineWidth - 2, color: 0xffffff });
      kickGlow.alpha = 0;
  ...
  function regenerateNecessaryObjects({FillGradient, Matrix, Container, Graphics, Text, Sprite}, {players, gameState}){
    ...
    function initHalo(){
      ...
      gr.stroke({ color: 0xffffff, width: thisRenderer.generalLineWidth, alignment: 0.5, alpha: 0.3 });
    ...
    function initBackground(){
      ...
      if (bgType==1){
        gr.x = -bgWidth; gr.y = -bgHeight;
        gr.roundRect(0, 0, 2*bgWidth, 2*bgHeight, bgCornerRadius);
        gr.fill({ texture: texture1, matrix: new Matrix().scale(2, 2), textureSpace: 'global' });
        gr.stroke({ color: 0xc7e6bd, width: thisRenderer.generalLineWidth, alignment: 0.5 });
        gr.moveTo(bgWidth, 0);
        ...
        gr.x = -10000; gr.y = -10000;
        gr.rect(0, 0, 20000, 20000);
        gr.fill({ texture: texture3, matrix: new Matrix().scale(2, 2), textureSpace: 'global' });
        const gr2 = new Graphics();
        gr2.x = 10000-bgWidth;
        gr2.y = 10000-bgHeight;
        gr2.roundRect(0, 0, 2*bgWidth, 2*bgHeight, bgCornerRadius);
        gr2.fill({ texture: texture2, matrix: new Matrix().scale(2, 2), textureSpace: 'global' });
        gr2.stroke({ color: 0xe9cc6e, width: thisRenderer.generalLineWidth, alignment: 0.5 });
        gr2.moveTo(bgWidth, 8);
        gr2.lineTo(bgWidth, 2*bgHeight);
        gr2.stroke({ fill: gradientFill, width: thisRenderer.generalLineWidth, alignment: 0.5 });
        var delta = bgWidth-bgGoalLine;
        if (bgGoalLine<bgCornerRadius) delta=0;
        var drawKickOff = function(color, x, ccw){
          gr2.beginPath();
          gr2.arc(bgWidth, bgHeight, bgKickOffRadius, -Math.PI/2, Math.PI/2, ccw);
          if (x!=0){ gr2.moveTo(x+bgWidth, 0); gr2.lineTo(x+bgWidth, 2*bgHeight); }
          gr2.stroke({ color: color, width: thisRenderer.generalLineWidth, alignment: 0.5 });
        };
    ...
    function initVertex(vertexObj){
      const gr = new Graphics();
      gr.stroke({ color: 0x000000, width: thisRenderer.generalLineWidth, alignment: 0.5 });
    ...
    function initJoint(jointObj, id){
      if (jointObj.color==-1) return;
      const { x: x0, y: y0 } = physicsState.discs[jointObj.d0].pos;
      const { x: x1, y: y1 } = physicsState.discs[jointObj.d1].pos;
      const gr = new Graphics();
      var info = customJointInfo[id] = { dx: x1-x0, dy: y1-y0, gr, color: jointObj.color };
      redrawJoint(info);
      gr.x = x0; gr.y = y0;
      stage2.addChild(gr);
    }

    function Animator(values){ this.values = values.slice(); }
    Animator.prototype = {
      eval: function(x){
        var idx = this.values.length-1;
        if (x<=this.values[0]) return this.values[1];
        if (x>=this.values[idx]) return this.values[idx-2];
        var min = 0, max = ((this.values.length / 5) | 0) - 1;
        do {
          var cur = (max+min)>>>1;
          if (x>this.values[5*cur]) min = cur+1;
          else max = cur-1;
        } while (min<=max);
    ...
    function CanvasText(lines, color){
      var arr = [];
      for (var i=0;i<lines.length;i++){
        var line = new Graphics();
        var text1 = new Text({ text: lines[i], style: { fontFamily: ["Arial Black", "Arial Bold", "Gadget", "sans-serif"], fontSize: 70, align: "center", fill: "#000000", fontWeight: "bold" } });
        text1.anchor.set(0.5);
        text1.x=7; text1.y=7;
        line.addChild(text1);
        var text2 = new Text({ text: lines[i], style: { fontFamily: ["Arial Black", "Arial Bold", "Gadget", "sans-serif"], fontSize: 70, align: "center", fill: Utils.numberToColor(color), fontWeight: "bold" } });
    ...
    CanvasText.alphaAnimator = new Animator([0, 0, 2, 1, 0, 0.35, 1, 0, 1, 0, 0.7, 1, 0, 0, 0, 1]);
    CanvasText.coordAnimator = new Animator([0, -1, 3, 0, 0, 0.35, 0, 0, 0, 0, 0.65, 0, 0, 1, 3, 1]);
    CanvasText.prototype = {
      calculateTime: function(){ return 2.31+0.1155*(this.arr.length-1); },
      ...
      destroy: function(){
        this.arr.forEach((x) => { x.destroy({ children: true, texture: true }); });
        this.arr = [];
      }
    }
    ...
    function initTexts(){
      ...
      textInfo = {
        time: 0, queue: [],
        timeUp: new CanvasText([TextMap[0], TextMap[1]], 16777215),
        redVictory: new CanvasText([TextMap[2], TextMap[3]], 15035990),
        redScore: new CanvasText([TextMap[4], TextMap[5]], 15035990),
        blueVictory: new CanvasText([TextMap[6], TextMap[7]], 625603),
        blueScore: new CanvasText([TextMap[8], TextMap[9]], 625603),
        gamePause: new CanvasText([TextMap[10], TextMap[11]], 16777215)
      };
    }
    ...
    function initLocationIndicator(id, pos, color, viewWidth, viewHeight){
    ...
    function initLocationIndicators(){
    ...
    function initChatIndicators(){
      players.forEach((player)=>{
        ...
        gr.visible = false;
        stage2.addChild(gr);
        chatIndicatorInfo[player.id] = { gr, active: false };
      });
    };

    function initFPSCounter(){
      fpsText = new Text({ text: "FPS: 60", style: { fontFamily: ["sans-serif"], fontSize: 16, fill: "#00FF00", fontWeight: "bold", stroke: { color: 0x000000, width: 3 } } });
      fpsText.x = -params.canvas.width/2 + 20;
      fpsText.y = -params.canvas.height/2 + 20;
      fpsText.visible = thisRenderer.showFPS;
      stage.addChild(fpsText);

      inputLagText = new Text({ text: "Input Lag: 0.00ms", style: { fontFamily: ["sans-serif"], fontSize: 16, fill: "#FFFF00", fontWeight: "bold", stroke: { color: 0x000000, width: 3 } } });
      ...
      netPingText = new Text({ text: "Ping: -- ms", style: { fontFamily: ["sans-serif"], fontSize: 16, fill: "#00FFFF", fontWeight: "bold", stroke: { color: 0x000000, width: 3 } } });
      ...
      netLossText = new Text({ text: "Packet Loss: 0%", style: { fontFamily: ["sans-serif"], fontSize: 14, fill: "#ff5555", fontWeight: "bold", stroke: { color: 0x000000, width: 3 } } });
  ...
  function addText(textObj){ textInfo.queue.push(textObj); }
  function resetTexts(){ textInfo.queue = []; textInfo.time = 0; }
  ...
  function updateText(deltaTime){
    ...
    if (textInfo.time<=queue[0].calculateTime()) return;
    textInfo.time = 0;
  ...
  function renderText(){
    if (textInfo.queue.length==0) return;
  ...
  function updateCameraOrigin(gameState, followDisc, geo, deltaTime){
    ...
    if (thisRenderer.followMode){
      ...
      if (followDisc && stadium.cameraFollow==1){
        ...
        pos = gameState.physicsState.discs[0].pos;
        x = pos.x;
        y = pos.y;
        if (followDisc){
    ...
    if (thisRenderer.restrictCameraOrigin){
      if (viewWidth>2*stadium.width) origin.x = 0;
      else if (origin.x+0.5*viewWidth>stadium.width) origin.x = stadium.width-0.5*viewWidth;
      else if (origin.x-0.5*viewWidth<-stadium.width) origin.x = -stadium.width+0.5*viewWidth;
      if (viewHeight - topPadding - bottomPadding > 2 * stadium.height) origin.y = (bottomPadding - topPadding) / 2;
      else if (origin.y+0.5*viewHeight-bottomPadding>stadium.height) origin.y = stadium.height-0.5*viewHeight+bottomPadding;
      else if (origin.y-0.5*viewHeight + topPadding <-stadium.height) origin.y = -stadium.height+0.5*viewHeight - topPadding;
    }
    if (Number.isNaN(origin.x)) origin.x = 0;
    if (Number.isNaN(origin.y)) origin.y = 0;
  ...
  function update(roomState, geo){
    ...
    segments.forEach((segment, id)=>{
      ...
      if (0*segment.curveF!=0){
        ...
        if (dx!=gr.dx || dy!=gr.dy){
          ...
          gr.lineTo(dx, dy);
          if (!segmentTransparent)
            gr.stroke({ color: segment.color, width: generalLineWidth, alignment: 0.5 });
        }
        gr.x = pos1.x; gr.y = pos1.y;
      } else {
    ...
    discs.forEach((disc, id)=>{
      ...
      try {
        ...
        const realPlayer = realRoom && realRoom.getPlayer ? realRoom.getPlayer(disc.playerId) : null;
        realKicking = realPlayer ? realPlayer.isKicking === true : false;
        const wasKicking = discInfo.isKicking === true;

        if (realKicking && !wasKicking && window.__bcOnKick) {
          try { window.__bcOnKick(realPlayer.id, realPlayer.name); } catch(e){}
        }
      } catch(e){}
      ...
      if (discInfo.strokeRadius !== disc.radius) {
        ...
        kickGlow.clear();
        if (squarePlayers) {
          playerStroke.rect(-disc.radius, -disc.radius, 2 * disc.radius, 2 * disc.radius);
          kickGlow.rect(-disc.radius, -disc.radius, 2 * disc.radius, 2 * disc.radius);
        } else {
          playerStroke.circle(0, 0, disc.radius);
          kickGlow.circle(0, 0, disc.radius);
        }
        playerStroke.stroke({ width: discLineWidth - 2, color: 0x000000 });
        kickGlow.stroke({ width: discLineWidth - 2, color: 0xffffff });
      ...
      discInfo.isKicking = realKicking;
      ...
      if (!discInfo.teamCache || discInfo.teamCache.teamId !== player.team.id || discInfo.teamCache.colors !== teamColors.inner || radiusChanged || discInfo.texturePath !== playerAvatarTexturePath){
        ...
        discInfo.teamCache = { teamId: player.team.id, colors: teamColors.inner };
      }
  ...
  function updateGamePaused(gameState) {
    ...
    if (!paused) { pauseRect.visible = false; return; }
  ...
  function setGamePaused(pauseState) {
    if (pauseState==gamePaused) return;
    if (!pauseState) {
  ...
  function resizeCanvas(){
    ...
    if (isBorderless && targetRes && targetRes !== 'native' && targetRes !== 'custom') {
      ...
      if (!isNaN(w) && !isNaN(h) && w > 0 && h > 0) { logicalWidth = w; logicalHeight = h; }
    }
    ...
    if (needsRecenter || Math.abs(rendererObj.resolution - expectedResolution) > 0.001 || rendererObj.width !== expectedPhysicalWidth || rendererObj.height !== expectedPhysicalHeight){
      ...
      if (fpsText) { fpsText.x = -logicalWidth / 2 + 20; fpsText.y = -logicalHeight / 2 + 20; }
      if (inputLagText) { inputLagText.x = -logicalWidth / 2 + 20; inputLagText.y = -logicalHeight / 2 + 40; }
      if (netPingText) { netPingText.x = -logicalWidth / 2 + 20; netPingText.y = -logicalHeight / 2 + 60; }
      if (netGraphGfx) { netGraphGfx.x = -logicalWidth / 2 + 20; netGraphGfx.y = -logicalHeight / 2 + 80; }
      if (netLossText) { netLossText.x = -logicalWidth / 2 + 20; netLossText.y = -logicalHeight / 2 + 124; }
  ...
  function pingColor(ms){ return ms < 60 ? "#33ff33" : (ms < 120 ? "#ffff33" : "#ff3333"); }
  ...
  function renderNetGraphBars(){
    ...
    for (var i = startIndex; i < netHistory.length; i++){
      ...
      var x = slot * barWidth;
      var barHeight, color;
      if (sample < 0){ barHeight = NET_GRAPH_HEIGHT; color = 0xff2222; }
      else {
  ...
  function refreshNetHud(){
    if (netPingText){
      if (netHistory.length === 0){ netPingText.text = "Ping: -- ms"; netPingText.style.fill = "#00FFFF"; }
      else {
  ...
  this.initialize = function(){
    ...
    async function isWebGPUSupported(){
      ...
      try { const adapter = await navigator.gpu.requestAdapter(); return !!adapter; }
    ...
    async function createRenderer(){
      ...
      try { await rendererObj.init(rendererOptions); }
      catch (e) {
    ...
    scriptElem = loadScript("pixi.min.js", ()=>{
  ...
  this.fps = function(){ return 1/spf; };
  ...
  this.onVariableValueChange = function(addonObject, variableName, oldValue, newValue){
    if (addonObject!=thisRenderer) return;
    switch(variableName){
      case "squarePlayers": {
        redrawHalo();
        const discs = thisRenderer.room.state.gameState.physicsState.discs;
      ...
      case "resolutionScale":
        if (rendererObj) { rendererObj.resolution = window.devicePixelRatio * newValue; _regenerateNecessaryObjects(); }
        break;
      case "displayMode":
      case "resolution":
        resizeCanvas();
  ...
  this.onWheel = function(event){
    if (event.deltaY<0) return thisRenderer.zoomIn(event.offsetX, event.offsetY, thisRenderer.wheelZoomCoeff);
    else return thisRenderer.zoomOut(event.offsetX, event.offsetY, thisRenderer.wheelZoomCoeff);
  };
  ...
  this.setZoom = function(pixelCoordX, pixelCoordY, targetZoom) {
    ...
    scale = targetZoom;
    thisRenderer.zoomCoeff = scale;
    if (stage2 && stage2.scale) stage2.scale.set(targetZoom, targetZoom);
    return scale;
  };
  ...
  this.onKeyDown = function(e){
    switch(e.keyCode){
      case 107: { thisRenderer.zoomCoeff += 0.1; scale += 0.1; _regenerateNecessaryObjects(); break; }
      case 109: {
        thisRenderer.zoomCoeff -= 0.1;
        if (thisRenderer.zoomCoeff<=0){ thisRenderer.zoomCoeff = 0.01; scale = 0.01; }
        else scale -= 0.1;
        _regenerateNecessaryObjects();
        break;
      }
  ...
  this.setOrigin = (_origin)=>{ origin.x = _origin.x; origin.y = _origin.y; };
  ...
  this.takeSnapshot = function(){
    ...
    return {
      webGPU, extrapolation, showTeamColors, showAvatars, showPlayerIds, zoomCoeff, wheelZoomCoeff,
      resolutionScale, showChatIndicators, showFPS, showNetGraph, restrictCameraOrigin, followMode,
      followPlayerId, drawBackground, squarePlayers, currentPlayerDistinction, showInvisibleSegments,
      showVertices, generalLineWidth, discLineWidth, displayMode, resolution,
      scale, origin: JSON.parse(JSON.stringify(origin)), gamePaused
    };
  };
  ...
  this.bc = {
    ...
    getStage2:              () => stage2,
    getStage3:              () => stage3,
    getCustomDiscInfo:      () => customDiscInfo,
    getChatIndicatorInfo:   () => chatIndicatorInfo,
    getLocationIndicatorInfo: () => locationIndicatorInfo,
    getOrigin:              () => origin,
    getScale:               () => scale,
